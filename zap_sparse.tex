...
\begin{keywords}
Online advertising, smile detection, Zapping probability, user preference
\end{keywords}}
...
To study the spontaneous consumer ad-watching facial expressions, an interesting study is conducted by McDuff \textit{et al.}~\cite{McDuff13} who have collected naturalistic facial response from people watching the US presidential debate in 2012. They found out that voter preference can be predicted with an average accuracy of over 73\% based on facial responses. An AMFED dataset is introduced in~\cite{amfed}, provided with both spontaneous facial videos and self-report responses. Although this data do not concern consumer zapping behavior, it provides a large testbed for ``in-the-wild'' consumer ad-watching expression analysis. In a more realistic case, consumers may not be cooperative and can switch away from the viewing content at any moment. Thus, it is important and challenging for advertisers to retain consumers' attention during the course of an ad~\cite{Elpers03}. \Songfan{rephrase the following sentence, I did it earlier but copy failed on me} Teixeira \textit{et al.}~\cite{Teixeira12} incorporate joy and surprise expression from a Bayesian neural network framework to analyze the viewer's zapping decision. They found that the velocity of the joy response highly impacts the viewer's zapping behavior. They also suggests that attention paid to a commercial ad determines the interest of the viewer and retaining a viewer's attention can produce desirable communication effects. 
...
A facial expression data for online advertising were introduced in~\cite{Yang_FG15} and are used in our experiments. In this dataset, 51 subjects were invited to watch 12 ads, each of which ranges from 30 to 90 seconds. Among those subjects, 16 were female and 35 were male. Regarding the ethnicity, there are 9 Caucasians, 14 (American born) Asian, 18 (foreign) Asians, 8 Hispanic, and 2 African-American. When watching an ad, a view can choose to finish watching or zap before it ends. In either case, 30 seconds were allowed for the viewer to neutralize their emotions. The selected 8 ads covers three most popular categories including \textit{car} and \textit{fast food}. The ads were purposely selected to avoid gender bias and have various entertaining level based on their contents. Here, entertaining level is defined as a scalar from 0 to 100 with higher number represents more entertaining content overall. Table~\ref{table:ads} lists the ads information. The \textit{Car} ads are more entertaining than the \textit{Fast food} ads with \textit{Toyota} and \textit{Nissan} being the two most amusing ads. All ads are available on primary sites such as YouTube. The playback order for these ads were randomized for each subject to diminish the order effect. 
...
\multirow{4}{*}{Car}        & Toyota & I Wish  & 80 & 60			\\ %\cline{2-3}
                            & Honda  & We Know You  & 30 & 90 \\ %\cline{2-3}
                            & Chevy  & Wind Test  & 40 & 30  \\ %\cline{2-3}
                            & Nissan & Enough   & 90 & 30    \\ %\cmidrule{2-3}
\midrule 
\multirow{4}{*}{Fast Food} & Jack In The Box & Hot Mess               & 50 &  30 \\ %\cline{2-3}
                            & Subway                & New Footlong    & 40 & 30 \\ %\cline{2-3}
                            & Carl's Jr.            & Oreo Ice Cream  & 60 &  32 \\ %\cline{2-3}
                            & Pizza Hut             & Make It Great   & 30 & 30 \\ %\cmidrule{2-3}
\bottomrule
\end{tabular}
...
\begin{itemize}
\item It is shown by Elper \textit{et al.}~\cite{Elpers03} that the lack of \textit{entertaining} and/or \textit{informative} factors in ads are the two major reasons for zapping. For the ad selection in this dataset, we focus on entertaining factor only. As amusing ad is a prevalent ad style, ads with various entertaining level is available. 
...
\item In terms of facial expression response to ad-watching, smile directly reflects the entertaining factor of ads. As in our design, smile plays a dominant role in our ad collection. It is observed that more than 95\% of the expressions are smile in the entire SARA dataset. A similar observation is also made in the recently published online ad-watching dataset, AMFED~\cite{amfed}. 
...
Concretely, given a set of $n$ smile response time series segments, $\X=[\x_1, \cdots, \x_n]\in \mathcal{R}^{m\times n}$, we intend to represent it by a dictionary $\D=[d_1,\cdots,d_p] \in \mathcal{R}^{m\times p}$, \textit{i.e.} $\x_i\approx \D \alpha_i$, where the reconstruction coefficient vector $\alpha_i \in \mathcal{R}^p$ is sparse. Learning the dictionary $\D$ is achieved by the following optimization problem
...
Having learnt a responselet dictionary, we consider extracting the feature at every single frame of a sequence. At frame, say $k$, where $k>=p$, we divide the current sequence into responselet of size $p$ in a similar sliding window fashion. This results in $k-p+1$ responselets and then each responselet is reconstructed by $\D$. The reconstruction coefficients $\alpha$ can be efficiently computed by solving Eq.~(\ref{eq:learn_dict}) with $\D$ fixed using least-angle regression (LARS) algorithm~\cite{LARS}.
...
We test our model using leave-one-subject-out cross validation. In each fold, one subject is hold out for testing, and a dictionary is learnt (although we find no noticeable difference in the learnt dictionary in each fold). Then a binary SVM classifier is trained using feature extracted at all frames in the training set, and then tested on the held-out subject.

We have also included several other features as baselines for comparison. For $k$-th frame in the sequence, we compute the following feature from frame 1 to $k$: mean, max, variance of an smile response sequence. The smile dynamics means we directly take the length-normalized smile response as feature. Smile histogram, used in~\cite{Yang_TAC14}, counts the smile response temporally with $[0,0.1,0.2,\cdots,1]$ as the edges. For direct comparison, we also include the sparse coefficients feature without mean-pooling. This feature only considers the responselet at the current frame.
...
\noindent \textbf{MMZP Validation.} To validate the accuracy of MMZP, we first compare it with the actually user zapping behavior. In Fig.~\ref{fig:ad}, the mean MMZP (left axis) of all subjects for each ad is plotted and is compared with the ground-truth zapping behavior as shown in the bar plot of each grid (right axis). Note that the zapping behavior is denoted as the percentage of an ad being watched, which is recorded during data collection. In general, the fast decreasing pattern of MMZP (less likely to zap) correlates with zapping behavior occurring near the end of an ad (e.g., \textit{Toyota} and \textit{Nissan}). In other words, zapping near the end suggests that more interesting contents were found in the ad by the user.  This is validated by the zapping distribution in the bar plot and is congruent with the ad entertaining level in the experimental design. 
\begin{figure*}[t]
	\centering
...
\noindent \textbf{User Preference} \Songfan{reword some sentences to focus on user preference} Targeted advertising has proven to be effective in online marketing, as evidenced by the dissemination of online personalized recommendations. We argue that the proposed MMZP can also be used as a user preference metric and therefore, has the potential to help with better advertising. In Fig.~\ref{fig:adcat_id} we show three subjects' MMZP for watching ads from two categories. It can be seen from the MMZP trend that subject 22 and 57 prefer \textit{Car} ads more than subject 63. On the other hand subject 63 is more interested in \textit{Fast food} than the other two subjects. This observation is also demonstrated by the corresponding zapping distribution. Thus, a better understanding of personal zapping behavior is essential for precise ad targeting.
