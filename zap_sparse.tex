...
% The paper headers
\markboth{IEEE Intelligent System}%
...
\begin{abstract}
%\boldmath
The surge of multimedia contents being made available online has attracted more users. As a new advertising platform, more advertisements have been presented online due to the increasing user population and reduced advertising cost. On the other hand, a large portion of revenue for major companies such as Google and Facebook is created by online advertisements. Hence, for both advertiser and advertisement hosts, effective advertisements targeting and delivering is of great interest and importance. In this paper, we quantify users' advertisement viewing experience based on their facial expression responses and propose a metric termed moment-to-moment zapping probability (MMZP) for predicting users' ``zapping'', i.e. ``skipping'' behavior. Insights are discovered by analyzing MMZP and such knowledge may help improve the advertising effectiveness.

\end{abstract}
\begin{keywords}
Online advertising, user behavior, facial expression, zapping probability
\end{keywords}}
...
%The performance of our smile intensity algorithm in terms of the ROC curve and samples of intensity estimation output are shown in Fig.~\ref{fig:smile_response}. We direct interested readers to~\cite{Yang_TAC14} for a more detailed description on how to estimate the smile intensity. More results are shown in Fig.~\ref{fig:smile_ex}. 
\end{itemize}
